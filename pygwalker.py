...
def get_gemini_client(api_key):
    ...
    return genai.GenerativeModel("gemini-1.5-flash") # 使用 gemini-1.5-flash 或 gemini-pro
...
def get_gemini_executive_analysis(api_key, executive_role_name, full_prompt):
    ...
    try:
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel("gemini-1.5-flash") # 使用 gemini-1.5-flash 或 gemini-pro
...
# --- 【新功能】Pygwalker 視覺化函式 ---
def display_pygwalker_chart(df: pd.DataFrame):
    """
    在 Streamlit 中顯示 Pygwalker 的互動式圖表介面。
    """
    st.subheader("🔍 互動式資料探索 (Pygwalker)")
    st.info("您可以基於上方 AI 的分析，在此處拖放欄位，自行探索資料並驗證洞見。")
    with st.expander("展開/收合 Pygwalker 探索介面", expanded=False):
        try:
            # 產生 pygwalker 的 HTML
            ...
            st.error(f"載入 Pygwalker 時發生錯誤: {e}")
            ...
            if st.session_state.executive_workflow_stage == "completed" or st.session_state.sp_workflow_stage == "completed":
                ...
                with st.container():
                    ...
                    if st.session_state.get("uploaded_file_path"):
                        try:
                            df_for_pyg = pd.read_csv(st.session_state.uploaded_file_path)
                            display_pygwalker_chart(df_for_pyg)
                        except Exception as e:
                            st.error(f"為 Pygwalker 載入資料時發生錯誤: {e}")
                # --- ▲▲▲ Pygwalker 區塊結束 ▲▲▲ ---
